...
def create_material() -> dict:
    ...
    try:
        with Session(engine) as session:
            ...
            id_material = id_max_material + 1 if id_max_material is not None else 1
            stmt = DB_materials(
        ...
        logger.error({'materials_(POST)': 'error in DB'}) # pragma: no cover
        return jsonify({'materials_(POST)': 'error in DB'}), 400 # pragma: no cover
...
def materials() -> List[Dict]:
    ...
    if available and available != 'all':
        return jsonify({'materials': 'misstake in args'}), 400
    try:
        with Session(engine) as session:
            stmt = (
                select(
        ...
        return jsonify({'materials': 'error in DB'}), 400 # pragma: no cover
